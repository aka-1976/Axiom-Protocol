...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    {
        let weights_path = std::path::PathBuf::from("weights.bin");
...

    {
        ...
        let status_route = warp::path!("v1" / "status")
            ...
            .and_then(move |addr: Option<SocketAddr>| {
                ...
                async move {
                    let peer_addr = addr.unwrap_or_else(|| "127.0.0.1:0".parse().unwrap());
...
async fn handle_rejection(err: warp::Rejection) -> Result<impl warp::Reply, std::convert::Infallible> {
    if err.find::<TooManyRequests>().is_some() {
        Ok(warp::reply::with_status(
            warp::reply::json(&serde_json::json!({"error": "Too Many Requests", "retry_after_secs": 60})),
